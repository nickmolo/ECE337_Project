...
COMPONENT_FILES	:= buffer.sv clockdivider.sv comparator.sv flex_counter.sv hs_sr.sv output_controller.sv outputMux.sv pixel_gen.sv sr_mux.sv stage_2.sv stageOne.sv tmds_controller.sv TMDS_Encoder.sv timertop.sv addr_bit_flip.sv stp_sr.sv

# Specify the name of the top level file (do not include the source folder in the name)
# NOTE: YOU WILL NEED TO SET THIS VARIABLE'S VALUE WHEN WORKING WITH HEIRARCHICAL DESIGNS
# AND THE AUTOMATED GRADING SYSTEM
TOP_LEVEL_FILE	:= hdmi_transmitter.sv
...
#	@$(SIMULATE) -t ps -do wave.do $(S_WORK_LIB).$(TB_MODULE)
...
mapped/$(TOP_MODULE).v: source/$(TOP_LEVEL_FILE) $(addprefix source/,$(COMPONENT_FILES))
	...
	@$(MAKE) --no-print-directory syn_mapped MAIN_FILE='$(TOP_LEVEL_FILE)' DEP_SUB_FILES='$(COMPONENT_FILES)' CLOCK_NAME='system_clk' CLOCK_PERIOD='4' > $(TOP_MODULE).log
